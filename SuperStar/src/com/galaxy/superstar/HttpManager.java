...
public class HttpManager {
	...
	static public String url = "http://115.28.52.64:9989";
...
//				urlCon.setDoOutput(true);
   ...
   static public String syncHttpsPost(String urlString, final List<NameValuePair> postData) {
	...
	static public InputStream syncPost(final String urlString, final byte[] postData, boolean encrypt) {
		try {
			return syncPost(urlString, new ByteArrayEntity(postData), encrypt);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	}
	static private String syncPost(final String urlString, final StringEntity postEntity, boolean encrypt, String charset) {
		HttpParams httpParams = new BasicHttpParams();
		HttpConnectionParams.setConnectionTimeout(httpParams, timeoutConnection);
		HttpConnectionParams.setSoTimeout(httpParams, timeoutSocket);
		/* 建立HTTP Post连线 */
	...
	static private InputStream syncPost(final String urlString, final ByteArrayEntity postEntity, boolean encrypt) {
		HttpParams httpParams = new BasicHttpParams();
		HttpConnectionParams.setConnectionTimeout(httpParams, timeoutConnection);
		HttpConnectionParams.setSoTimeout(httpParams, timeoutSocket);
		/* 建立HTTP Post连线 */
		...
		try {
			...
			// 取出回应字串
				HttpEntity entity = httpResponse.getEntity();
				if(entity != null) {
					InputStream httpContent	= entity.getContent();
	...
	static public InputStream syncPost(final String url, final byte[] postData) {
		return syncPost(url, postData, false);
	}
